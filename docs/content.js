(function () {
  ...
  pages.home = page("Blockified Engine", "Documentation · Guides · Wiki", "Build Friday Night Funkin' songs, stages, machines, and world scenes inside Minecraft.", ["2.3.4bbs", "NeoForge 1.21.1", "Singleplayer + LAN"],
...
function play:onClick()
  ...
  pages.animations = page("Animations", "Creation guide", "Create BBS character mappings and Sparrow XML sprite animations.", ["BBS states", "Sparrow XML", "JSON", "Lua"],
    ...
    section("bbs-map", "2. Map a BBS character", `<p>Create <code>animations/my-character.json</code> in a complete pack. Add <code>my-character-opp.json</code> only when the opponent needs a different form or mapping. Freeplay uses the animation definition's <code>icon</code> when no Psych/V-Slice character JSON exists under the same character name; when both exist, the character JSON and its health icon take priority.</p>${code("json", `{
  "bbsForm": "My BF form",
  "vocals_file": "bf",
  ...
  "animations": {
    ...
    section("bbs-use", "4. Use and test it", `<ol><li>Open the Character Editor. Its Character, Model, and Animations tabs edit either the Player or Opponent role while the BBS form preview stays on the left. In a bundled mod world, it edits that pack's <code>animations/</code> folder.</li><li>Use <strong>Load</strong> to choose a character from the searchable list. A bundled form is loaded for preview even when it is not installed in BBS's normal config folder, and a private copy is imported into BBS's <strong>Recent</strong> category. Distinct player and opponent bundles are both imported; loading the same bundle again replaces Blockified's previous Recent copy.</li><li>Choose animations from the searchable left-hand list and preview them on the right. The configured Left/Down/Up/Right keys play direction states; non-looping idle/idle2 mappings alternate at a 120 BPM preview beat. Hover the Anim button and use the wheel for quick cycling without opening the list.</li><li><strong>Save</strong> writes the player JSON and any meaningfully configured opponent JSON together. For a custom definition it also bundles the selected BBS model and texture.</li><li>On the Model tab, <strong>Skin / Model Rules</strong> controls whether player overrides are allowed and selects the form, performer, or a named Minecraft account as the authored skin/model source.</li><li>Under Visual settings, <strong>Player Anims</strong> controls the BF/player chart role and <strong>Opponent Anims</strong> controls Dad/opponent. Both selectors independently support the form skin, current player skin, a PNG file with Slim/Wide selection, or a Minecraft account name. A locked or incompatible skin button is disabled. Main-model texture keyframes cannot cover a selected Minecraft skin; other animation keyframes still run.</li><li>Use <strong>Play Animation</strong> with <code>attack</code>, or call <code>characterPlayAnim(tag, 'attack')</code>. Run <code>/fnf reload animations</code> after editing files externally.</li></ol>${callout("Names are open-ended", "Idle and sing directions drive normal gameplay, but custom names are supported. The JSON key is the name used by events and Lua.")}`),
...
dancer:playAnimation('idle')`)),
  ...
  pages.events = page("Chart events", "Documentation · Gameplay", "Blockified events and modifications to compatible Psych events.", ["Minecraft", "3D camera", "Characters"],
    section("new", "Blockified events", `<div class="api-list">
      ...
      ${api("Camera Rotation 3D", "V1 pitch; V2 yaw; V3 roll; V4 easing; V5 duration in seconds (blank = 0.5s); V6 layer. Blank/normal/orbit uses the original layer, which rotates position around the pivot during Camera Orbit. camera/view/local uses an independent view-only layer that keeps working while orbiting. Both layers have separate tween state.", "Event")}
...
setObjectOrder('badge', 10)`)}`),
    section("actions", "Machine actions", `<div class="api-list">
      ...
      ${api("machine.getWeeks()", "Returns Psych-compatible week metadata, including songs, difficulties, characters, visibility flags, weekBefore, image, and playable state.", "Machine")}
      ${api("machine.getWeek(id) / machine.hasWeek(id)", "Reads or checks one discovered week.", "Machine")}
      ${api("machine.openScreen(name)", "Switches to screens/name.lua without cutting shared tagged audio or clearing machineData.", "Machine")}
      ${api("machine.getScreen() / machine.getScreens()", "Reads the current Lua page or lists main plus every screens/*.lua page.", "Machine")}
...
section("screens", "Multiple Lua screens and shared audio", `<p>Keep the entry page at <code>menu.lua</code> and put extra pages at <code>screens/&lt;name&gt;.lua</code>. Call <code>machine.openScreen('name')</code> to navigate, or <code>machine.openScreen('main')</code> to return. Widgets, tweens, timers, and page callbacks reset; <code>machineData</code> and tagged sounds remain in the same sandboxed runtime. Looping music therefore continues across pages and follows live master-volume changes. Use <code>soundExists(tag)</code> to avoid restarting a shared track when the main page reopens.</p>${code("lua", `-- menu.lua
if not soundExists('menuMusic') then
  playSound('menuMusic', 'sounds/menu', 0.7, true)
end
local story = ui.button('story', 'Story Mode', 0.5, 0.5, 180, 24)
function story:onClick() machine.openScreen('story') end

-- screens/story.lua
for _, week in ipairs(machine.getWeeks()) do
  -- week.songs[n].id/icon/color/playable
end
local back = ui.button('back', 'Back', 0.5, 0.9, 120, 24)
function back:onClick() machine.openScreen('main') end`)}`),
...
dancer:playAnimation('idle')`)}`),
    section("sound", "Sound", `<p>Menus can play OGG sound effects and music. Files resolve like every other menu asset: the path is tried in the machine folder first, then in the owning mod's root, so shared sounds can live in <code>&lt;mod&gt;/sounds/</code>. The <code>.ogg</code> extension is optional, matching Psych's convention.</p><div class="api-list">
      ...
      ${api("soundExists(tag) / getSoundVolume(tag)", "Checks a shared tagged sound or reads its script-level volume.", "Machine")}
      ${api("setSoundPitch/getSoundPitch; setSoundTime/getSoundTime", "Changes or reads tagged sound pitch and playback time (milliseconds).", "Machine")}
...
function play:onClick()
  ...
  pages.commands = page("Commands and recovery", "Documentation · Tools", "Open editors, reload content, and recover from a stuck session.", ["Commands"],
    ...
    section("reload", "Reload", `<p><code>/fnf</code> commands require cheats in singleplayer or operator permission on a server.</p><div class="api-list">${api("/fnf reload all", "Reloads all supported categories.", "Command")}${api("/fnf reload songs", "Reloads song library.", "Command")}${api("/fnf reload skins|splashes|animations|icons|hitsounds|fonts|options|scores", "Reloads one category.", "Command")}</div>`),
  ...
  pages.formats = page("Paths and formats", "Documentation · Tools", "Quick reference for Blockified-owned locations.", ["Reference"],
    ...
    section("mods-settings", "Mods settings and pack metadata", `<p>The <strong>Mods</strong> settings page always includes <code>config/fnfmod/mods</code>. You may add either one complete mod folder directly or a container <code>mods</code> folder. Direct mods are detected from <code>pack.json</code>/<code>pack.png</code> or standard content folders; containers expand every non-content child and scan each as an isolated pack. Psych-compatible metadata supplies names, descriptions, and icons. Source labels show only the final three path folders.</p><p>Select a source or child mod to show its icon, name, description, shortened path, and permissions in the scrollable left panel. Permissions remain at the bottom; click or drag across resource buttons to toggle several categories. Enabled buttons are bright and disabled buttons are gray. A pack inherits its source permissions until given its own override. Images intentionally blocked by this permission or by Minecraft-look isolation are hidden; the missing-asset texture is reserved for files that were allowed but genuinely could not be found or decoded.</p>${callout("No metadata required", "A child folder does not need pack.json or pack.png to remain isolated. Its folder name becomes the display name and the details panel reports that no description/icon was provided.")}`),
    section("polymod-meta", "V-Slice / Polymod metadata", `<p>V-Slice packs using <code>_polymod_meta.json</code> and <code>_polymod_icon.png</code> are first-class Mods entries. Blockified reads their title, description, <code>mod_version</code>, license, icon, and contributor array while preserving Psych <code>pack.json</code>/<code>pack.png</code> support.</p>`),
    section("credits-menu", "Credits menu", `<p>Use the small <strong>C</strong> button at the bottom-right of the Song Menu's <strong>Settings</strong> tab. The scrollable credits combine Blockified and the upstream projects with installed V-Slice contributor arrays and Psych <code>data/credits.txt</code> files. Psych rows support <code>name::icon::description::link::colorHex</code>, including credit icons, colors, descriptions, and clickable HTTP(S) links. Hardcoded engine credits are intentionally not clickable.</p>`),
